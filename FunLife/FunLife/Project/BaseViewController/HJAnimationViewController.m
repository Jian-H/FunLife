...
@interface HJAnimationViewController () <HJItemViewDelegate>

@property (nonatomic, assign) NSInteger currentTag;

@end

@implementation HJAnimationViewController
...
- (void)configViews {
    ...
    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"FL_HomePageBackground"]];
    NSArray *dataArray = @[@"FL_Menu1", @"FL_Menu1", @"FL_Menu1", @"FL_Menu1", @"FL_Menu1"];
    ...
    for (NSInteger i = 0;i < PHOTONUM;i++) {
        ...
        HJItemView *view = [[HJItemView alloc] initWithNormalImage:dataArray[i] highlightedImage:[dataArray[i] stringByAppendingFormat:@"%@", @"_hover"] tag:TAGSTART+i title:nil];
...
#pragma mark - HJItemViewDelegate
- (void)didTapped:(NSInteger)index {
    
    if (self.currentTag  == index) {
        NSLog(@"自定义处理事件");
        
        if ([self.delegate respondsToSelector:@selector(animationViewControllerDidSelectedItemIndex:)]) {
            [self.delegate animationViewControllerDidSelectedItemIndex:index-TAGSTART];
        }
